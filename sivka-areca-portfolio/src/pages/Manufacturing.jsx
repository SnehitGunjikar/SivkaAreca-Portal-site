...
const ProcessStep = ({ step, index, isActive, onClick }) => {
  return (
    <motion.div
      ...
      className={`cursor-pointer transition-all duration-300 rounded-2xl ${isActive ? 'ring-2 ring-offset-2 ring-offset-white ring-brand-500 shadow-lg' : ''
        }`}
    >
      <SpotlightCard
        className={`rounded-2xl border p-6 text-center shadow-md transition-all duration-300 ${isActive
...
export default function Manufacturing() {
  ...
  return (
    <>
      <Helmet>
        <title>Our Manufacturing Process | Sivka Areca Enterprises</title>
        <meta
          name="description"
          content="Standardized manufacturing workflow ensuring precision in steel fabrication, welding quality, following IS/BS/ASTM standards."
        />
        <link rel="canonical" href="https://www.sivkaareca.com/manufacturing" />
      </Helmet>
          ...
          <div className="relative w-full h-full overflow-hidden">
            ...
            <div className="absolute inset-0">
              {manufSlides.map((image, index) => (
                <div
                  key={index}
                  className={`absolute inset-0 transition-opacity duration-1000 ease-in-out bg-center md:bg-top bg-no-repeat bg-cover md:bg-fixed ${index === currentSlide ? 'opacity-100' : 'opacity-0'
                    }`}
                  style={{
            ...
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-1 sm:space-x-2 z-20">
              {manufSlides.map((_, index) => (
                <button
                  ...
                  className={`w-1.5 h-1.5 sm:w-2 sm:h-2 md:w-3 md:h-3 rounded-full transition-all duration-300 shadow-lg ${index === currentSlide
                    ? 'bg-white scale-125'
